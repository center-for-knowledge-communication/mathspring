...
public class TTUtil {
    ...
    public static final String LANDING_REPORT_STUDENTS_QUERY ="Select studId AS studentId,concat(s.fname,' ',s.lname) As studentName, s.userName As userName,count(problemId) AS noOfProblems from student s,studentproblemhistory sh where s.id=sh.studId and s.classId=(:classId) and sh.mode != 'demo' GROUP BY studId order by studId;";
    public static final String LANDING_REPORT_EVENTS_QUERY ="select distinct s.id AS studentId, s.classid, e.clickTime as clickTime, e.action as action, e.sessNum as sessNum, e.probElapsed as probElapsed, e.elapsedTime as elapsedTime from student as s, class as c, eventlog as e where s.classid = (:classId) and e.studId = s.id and e.clickTime > (:ts) order by e.studId, e.time DESC;";
