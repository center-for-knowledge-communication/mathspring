...
*/
    ...
    public String getCohortTeachersClasses(String cohort, String teacherId, String classId, String filter) throws SQLException {
    	...
    	try {
            ...
            String q = "select tmc.researchcohortid as cohortId, tmc.name as cohortName, t.ID as teacherId, t.username as username, fname as firstName ,lname as lastName, c.id as classId, c.name as className from teacher t, teacher_map_cohort tmc, class c where t.ID = tmc.teacherid and c.teacherId = t.ID order by tmc.researchcohortid,lname asc;";
...
*/        
    ...
    public String getCohorts(String reportType,  String lang, String filter) {
    	...
    	try {
        	Connection conn = connection.getConnection();
        	String q = "select distinct tmc.researchcohortid as cohortId, coh.name as cohortName  from teacher_map_cohort tmc INNER JOIN cohort as coh ON tmc.researchcohortid=coh.researchcohortid  order by tmc.researchcohortid asc;";
        	stmt = conn.prepareStatement(q);
            rs = stmt.executeQuery();
        	JSONArray cohortsArr = new JSONArray();
            JSONObject cohortJson = new JSONObject();
            while (rs.next()) {
                cohortJson.put("cohortId", rs.getInt(1));
                cohortJson.put("cohortName", rs.getString(2));
    ...
    public String getTeacherLogins(Connection conn, int cohortId) throws SQLException {
        ...
        String ticks = "";
        try {
            String q = "SELECT teacherlog.teacherId, userName as uname, COUNT(*) AS logins FROM teacher, teacherlog, teacher_map_cohort where teacherlog.teacherId = teacher_map_cohort.teacherid and teacher_map_cohort.researchcohortid = ? and teacher.id = teacherlog.teacherId and action = 'login' GROUP BY teacherId;";
            stmt = conn.prepareStatement(q);
    ...
    public String getTeacherActivityMetrics(Connection conn, int cohortId) throws SQLException {
        ...
        try {
            String q = "SELECT teacherlog.teacherId, userName as uname, action, count(*) as total FROM teacher, teacherlog, teacher_map_cohort where teacherlog.teacherId = teacher_map_cohort.teacherid and teacher_map_cohort.researchcohortid = ? and teacher.id = teacherlog.teacherId GROUP BY teacherId,action;";
            stmt = conn.prepareStatement(q);
            stmt.setInt(1, cohortId);
            rs = stmt.executeQuery();
        	JSONArray resultArr = new JSONArray();
    ...
    public String getCohortWeeks(Connection conn, int cohortId, String filter) throws SQLException {
        ...
        try {
            String q_start = "SELECT startdate, YEAR(startdate) year_no, WEEK(startdate) week_no FROM cohort where cohort.researchcohortid = ?";
            ...
            String q_latest = "SELECT YEAR(teacherlog.time) year_no, WEEK(teacherlog.time) week_no, time as latestdate  FROM teacher, teacherlog, teacher_map_cohort where teacherlog.teacherId = teacher_map_cohort.teacherid and teacher_map_cohort.researchcohortid = ? and teacher.id = teacherlog.teacherId ORDER BY time desc;";
    ...
    public String createCohortSlice(Connection conn, int cohortId, String filter) throws SQLException {
        ...
        PreparedStatement stmt = null;
        try {
            String q1 = "select t.ID as teacherId, t.userName as uname, tmc.researchcohortid as cohortId,  c.id as classId, YEAR(?) as year_no, WEEK(?) as week_no  from teacher t, teacher_map_cohort tmc, class c where tmc.researchcohortId = ? and t.ID = tmc.teacherid and c.teacherId = t.ID order by tmc.researchcohortid,lname asc;";
    ...
    public String insertCohortSlice(Connection conn, int cohortId, int teacherId, String uname, int classId, Timestamp week_startdate) throws SQLException {
    	
    	int result = 0;
    	String resultMessage = "Success";
        PreparedStatement stmt = null;
        try {
            String q = "INSERT into teacherlogslice (cohortId,teacherId,teacherUsername,classId, week_startdate) VALUES (?,?,?,?,?);";
    ...
    public String updateCohortSlice(Connection conn, int cohortId, int teacherId, String teacherUsername, int classId, Timestamp week_startdate, int logins, int actions, int logouts) throws SQLException {
        ...
        try {
        	if (classId == 0) {
        		q = "UPDATE teacherlogslice SET logins = ?,actions = ?,logouts = ? where cohortId = ? and teacherId = ? and teacherUsername = ? and classId = ? and week_startdate = ?;";
	            stmt = conn.prepareStatement(q);
            ...
            if (result == 0) {
                String q2 = "INSERT into teacherlogslice (cohortId,teacherId,teacherusername, classId, week_startdate, logins, actions, logouts) VALUES (?,?,?,?,?,?,?,?);";
    ...
    public String updateAllCohortSlicesTeacherActivity(Connection conn, int cohortId, String filter) throws SQLException {
    	...
    	for (int rpt_week_no = 1;rpt_week_no<=weeksToReport;rpt_week_no++) {

    	
// updateCohortSliceTeacherActivity(conn, cohortId, filter);
            ...
            PreparedStatement stmt = null;
            try {
                String q = "SELECT teacherlog.teacherId as teacherId, teacher.UserName as teacherUsername, teacherlog.action as action, count(*) as total  FROM teacher, teacher_map_cohort, teacherlog where teacherlog.teacherId = teacher_map_cohort.teacherid and teacher_map_cohort.researchcohortid = ? and teacher.id = teacherlog.teacherId and teacherlog.time BETWEEN ? AND ? GROUP BY teacherId,action;";
//                String q = "SELECT YEAR(teacherlog.time) year_no, WEEK(teacherlog.time) week_no, time, teacherlog.teacherId as teacherId, userName as teachername, COUNT(*) AS logins FROM teacher, teacherlog, teacher_map_cohort where teacherlog.teacherId = teacher_map_cohort.teacherid and teacher_map_cohort.researchcohortid = ? and teacher.id = teacherlog.teacherId and action = 'login' and YEAR(teacherlog.time) = ? and WEEK(teacherlog.time) = ? GROUP BY year_no, week_no, teacherId;";
...
//and e.time BETWEEN (:tsFromDate) AND (:tsToDate)
    ...
    public String getCohortSlice(Connection conn, int cohortId, String filter) throws SQLException {
        ...
        PreparedStatement stmt = null;
        try {
            //String q = "SELECT YEAR(?) year_no, WEEK(?) week_no, tls.teacherId as teacherId, userName as uname, tls.logins as logins, tls.actions as actions, tls.logouts as logouts, time FROM teacher, teacherlog, teacher_map_cohort where tls.teacherId = teacher_map_cohort.teacherid and teacher_map_cohort.researchcohortid = ? and YEAR(teacherlog.time) = ? and WEEK(teacherlog.time) = ? GROUP BY year_no, week_no, teacherId;";
            String q = "SELECT tls.year_no as year_no, tls.week_no as week_no,tls.teacherId as teacherId, tls.teacherUsername as uname, tls.logins as logins, tls.actions as actions, tls.logouts as logouts FROM teacherlogslice as tls, teacher, teacherlog, teacher_map_cohort where tls.teacherId = teacher_map_cohort.teacherid and teacher_map_cohort.researchcohortid = ? and tls.year_no = ? and tls.week_no = ? GROUP BY year_no, week_no, teacherId;";
            stmt = conn.prepareStatement(q);
            stmt.setInt(1, cohortId);
            stmt.setInt(2, year);
            stmt.setInt(3, week);
    ...
    public String getCohortRangeSlices(Connection conn, int cohortId, String filter) throws SQLException {
    	...
    	for (int rpt_week_no = 1;rpt_week_no<=weeksToReport;rpt_week_no++) {
	        ...
	        try {
	            //String q = "SELECT YEAR(?) year_no, WEEK(?) week_no, tls.teacherId as teacherId, userName as uname, tls.logins as logins, tls.actions as actions, tls.logouts as logouts, time FROM teacher, teacherlog, teacher_map_cohort where tls.teacherId = teacher_map_cohort.teacherid and teacher_map_cohort.researchcohortid = ? and YEAR(teacherlog.time) = ? and WEEK(teacherlog.time) = ? GROUP BY year_no, week_no, teacherId;";
	            String q = "SELECT tls.teacherId as teacherId, tls.teacherUsername as uname, tls.logins as logins, tls.actions as actions, tls.logouts as logouts FROM teacherlogslice as tls, teacher, teacherlog, teacher_map_cohort where tls.teacherId = teacher_map_cohort.teacherid and teacher_map_cohort.researchcohortid = ? and tls.week_startdate BETWEEN ? AND ? GROUP BY teacherId;";
	            stmt = conn.prepareStatement(q);
	            stmt.setInt(1, cohortId);
	            stmt.setTimestamp(2, tsFromDate);
	            stmt.setTimestamp(3, tsToDate);
    		...
    		weekJson.put("Week" + String.valueOf(rpt_week_no), weekArr);
